...
struct Ray {
	...
	inline Ray(const glm::vec3 &t_origin, const glm::vec3 &t_dir) : m_origin(t_origin), m_dir(t_dir) {}
...
struct AABB {
	...
	inline AABB(const glm::vec3 &t_min, const glm::vec3 &t_max) : m_min{t_min}, m_max{t_max} {}
	...
	inline void Expand(const glm::vec3 &vec) {
		m_min = glm::min(vec, m_min);
		m_max = glm::max(vec, m_max);
	}
...
struct Triangle {
	...
	inline AABB GetAABB() const {
		return {glm::min(m_positions[0], glm::min(m_positions[1], m_positions[2])),
